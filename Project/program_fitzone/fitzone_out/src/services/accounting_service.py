...
class AccountingService:
    ...
    def total_ingresos(self):
        ...
        from_entries = sum(
            e.monto for e in self.entries
            if e.tipo == "ingreso" and e.estado == "pagado"
        )
        from_records = sum(
            r.monto for r in self.records
            if r.estado == AccountRecord.ESTADO_PAGADO
        )
        return from_entries + from_records
    ...
    def total_por_usuario(self, user_id):
        """Total pagado por un usuario específico (busca en records)."""
        uid = str(user_id)
        return sum(
            r.monto for r in self.records
            if str(r.id_cliente) == uid and r.estado == AccountRecord.ESTADO_PAGADO
        )

    def reporte_ingresos(self, desde=None, hasta=None):
        """
        Reporte consolidado de ingresos con filtro de fechas opcional.
        desde / hasta: strings 'YYYY-MM-DD'.
        """
        registros = self.get_pagados()
        if desde:
            registros = [r for r in registros if r.fecha >= desde]
        if hasta:
            registros = [r for r in registros if r.fecha <= hasta]
        total = sum(r.monto for r in registros)
        return {
            "total": total,
            "cantidad_cobros": len(registros),
            "promedio": total / len(registros) if registros else 0,
