...
class PaymentService:
    ...
    def __init__(self, auth_service=None, membership_service=None,
        ...
        # Inyección de AccountingService para registro automático
    ...
    def verify_payment(self, payment_id):
        for user in self.auth.get_users():
            for payment in user.payments:
                ...
                # Registrar automáticamente en contabilidad
                self._registrar_en_contabilidad(user, payment)
    ...
    def _registrar_en_contabilidad(self, user, payment):
        """
        Escribe automáticamente en accounting.json cuando se verifica un pago.
        El fallo contable NO revierte el pago; solo se loguea.
        """
        if self._accounting is None:
            return
        try:
            concepto = (
                f"Membresía {payment.get('membership_name', 'N/A')} - "
                f"{user.nombre} ({user.correo})"
            )
            registro = self._accounting.registrar_cobro(
                id_cliente=str(user.id_cliente),
                nombre_cliente=user.nombre,
                concepto=concepto,
                monto=float(payment.get("amount", 0)),
            )
            self._accounting.confirmar_pago(registro.id_registro)
            self.auth.add_log(
                f"Contabilidad actualizada: registro #{registro.id_registro} "
                f"para {user.nombre}"
            )
        except Exception as exc:
            self.auth.add_log(f"ADVERTENCIA: fallo al registrar en contabilidad: {exc}")
