...
# Actualizado: exportación CSV agregada manteniendo diseño original
...
class ReportsView(QWidget):
    ...
    def _build_members_tab(self):
        ...
        btn_csv.clicked.connect(self._export_members_csv)
        btn_row.addStretch()
    ...
    def _gen_members(self):
        ...
        self._last_members_data = rows

    def _export_members_csv(self):
        rows = getattr(self, "_last_members_data", None)
        if not rows:
            QMessageBox.warning(self, "Aviso", "Genera el reporte primero.")
            return
        now = datetime.now().strftime("%Y%m%d_%H%M")
        path, _ = QFileDialog.getSaveFileName(
            self, "Guardar CSV", f"reporte_membresias_{now}.csv", "CSV (*.csv)")
        if not path:
            return
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as f:
                w = csv.writer(f)
                w.writerow(["Nombre", "Correo", "Rol", "Plan", "Estado"])
                for r in rows:
                    w.writerow([r["nombre"], r["correo"], r["rol"], r["plan"], r["estado"]])
            QMessageBox.information(self, "Listo", f"CSV guardado en:\n{path}")
    ...
    def _build_activity_tab(self):
        ...
        btn_csv = QPushButton("⬇  Exportar CSV")
        btn_csv.clicked.connect(self._export_activity_csv)
        btn_row.addStretch()
        btn_row.addWidget(btn_csv)
        btn_row.addWidget(btn_gen)
        v.addLayout(btn_row)
    ...
    def _gen_activity(self):
        ...
        self._last_activity_data = rows

    def _export_activity_csv(self):
        rows = getattr(self, "_last_activity_data", None)
        if not rows:
            QMessageBox.warning(self, "Aviso", "Genera el reporte primero.")
            return
        now = datetime.now().strftime("%Y%m%d_%H%M")
        path, _ = QFileDialog.getSaveFileName(
            self, "Guardar CSV", f"reporte_pagos_{now}.csv", "CSV (*.csv)")
        if not path:
            return
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as f:
                w = csv.writer(f)
                w.writerow(["Usuario", "Correo", "Valor", "Método"])
                for r in rows:
                    w.writerow([r["usuario"], r["correo"], r["valor"], r["metodo"]])
            QMessageBox.information(self, "Listo", f"CSV guardado en:\n{path}")
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))
