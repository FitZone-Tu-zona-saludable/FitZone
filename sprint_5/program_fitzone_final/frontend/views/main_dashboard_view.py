...
class MainDashboardView(QWidget):
    ...
    def reload(self):
        ...
        user_id   = str(user.get('id', user.get('user_id', ''))).zfill(2)
        self.summary_name.setText(f"Bienvenido, {user_name}")
        self.summary_role.setText(
            f"Rol activo: {user.get('role', '').capitalize()}  ·  "
